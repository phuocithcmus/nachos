...
int findAndMarkEmptyPage() {
	return gPhysPageBitMap->Find();
}
void unmarkPage(int pos) {
	gPhysPageBitMap->Clear(pos);
}
...
AddrSpace::AddrSpace(char * filename)
{
	...
	for (i = 0; i < numPages; i++) {
		...
		pageTable[i].physicalPage = findAndMarkEmptyPage();
...
AddrSpace::~AddrSpace()
{
	for (int i = 0; i < numPages; i++)
		unmarkPage(pageTable[i].physicalPage);
	delete pageTable;
}
